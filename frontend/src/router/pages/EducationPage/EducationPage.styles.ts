...
import { GAME_WIDTH, GAME_MARGIN, PLAYER_SIZE, PLATFORM_HEIGHT } from './constants'
...
export const GameArea = styled.div<{ gameHeight: number }>`
  position: relative;
  width: ${GAME_WIDTH}px;
...
export const PlatformContainer = styled.div<{ left: number; top: number; width: number }>`
  position: absolute;
  left: ${({ left }) => left}px;
  top: ${({ top }) => top}px;
...
export const AnimatedPlayerWrapper = styled(animated.div)`
  position: absolute;
